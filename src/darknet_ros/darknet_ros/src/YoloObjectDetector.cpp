...
void YoloObjectDetector::init()
{
  ...
  nodeHandle_.param("subscribers/commond_reading/topic", topicname, std::string("YOLO_GO"));
  nodeHandle_.param("subscribers/commond_reading/queue_size", yoloQueueSize, 1);
  ...
  detectionImagePublisher_ = nodeHandle_.advertise<sensor_msgs::Image>(detectionImageTopicName,
                                                                       ...
                                                                       detectionImageLatch);
  //Fenny
  sub_merge = nodeHandle_.subscribe(topicname, yoloQueueSize, &YoloObjectDetector::MergeCallback, this);
...
//Fenny
void YoloObjectDetector::MergeCallback(const std_msgs::Int32::ConstPtr& msg){
  ROS_INFO("received from merge");
  isYoloRuning = true;
  object_id = msg->data;
}
...
void YoloObjectDetector::yolo()
{
  ...
  const auto wait_detect = std::chrono::milliseconds(3000);
  ...
  while (!demoDone_) {
    #if 0
    while (!getIsRuning()) {
      printf("Waiting for commond.\n");
      if (!isNodeRunning()) {
        return;
      }
      std::this_thread::sleep_for(wait_detect);
    }
    #endif
...
bool YoloObjectDetector::getIsRuning(void)
{
  return isYoloRuning;
}
